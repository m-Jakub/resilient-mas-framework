...
def collect_aggregated_axioms(
    ...
    for payload in private_thought_buffer.values():
        ...
        if hasattr(payload, "strategic_insight"):
            if payload.strategic_insight:
                _add_axioms([str(payload.strategic_insight)])
            if hasattr(payload, "audit") and isinstance(payload.audit, dict):
                _add_axioms(payload.audit.get("retrieved_axioms", []))
...
def collect_private_traces(
    ...
    for payload in private_thought_buffer.values():
        ...
        if hasattr(payload, "nl_instruction"):
            _add_trace(payload.nl_instruction)
